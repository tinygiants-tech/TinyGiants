...
```bash
Assets/
└── 📁 TinyGiants/                  # [프로젝트 루트] 모든 항목이 이 폴더 내에 포함됨
    ├── 📁 GameEventSystem/         # [CORE LOGIC] 불변의 플러그인 루트
...
| TinyGiants/TinyGiantsData/.../Database  | **커밋** | 실제 이벤트 에셋들이 포함되어 있습니다. 매우 중요한 데이터입니다. |
| TinyGiants/TinyGiantsData/.../FlowGraph | **커밋** | 시각적 로직 그래프가 포함되어 있습니다. 매우 중요한 데이터입니다. |
| TinyGiants/TinyGiantsData/.../CodeGen   | **커밋** | **권장.** 다시 생성할 수도 있지만, 커밋해 두면 다른 팀원들이 위자드를 실행할 필요 없이 프로젝트가 즉시 컴파일됩니다. |
