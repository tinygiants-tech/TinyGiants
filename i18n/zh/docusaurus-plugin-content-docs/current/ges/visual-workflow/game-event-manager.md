...
2. **关注点分离**：
   - **数据库**：存储事件定义（标识、名称、类型）。
   - **流程图**：存储逻辑节点（触发器、链、连接）。
3. **"保护区"**：默认情况下，所有资产都在 `Assets/TinyGiants/TinyGiantsData/GameEventSystem/` 中创建。
...
| **+ 创建新的** | 在 `TinyGiants/TinyGiantsData/GameEventSystem/Database` 文件夹中创建新的 `.asset` 数据库文件并在此处添加。 |
...
```
✅ 正确：Assets/TinyGiants/TinyGiantsData/GameEventSystem/
