...
function LanguageSwitcher() {
    ...
    const display = i18n.currentLocale === 'en' ? 'EN' : 'ZH';

    return (
      <div className={styles.langWrapper} ref={dropdownRef}>
          <button className={styles.langButton} onClick={() => setIsOpen(!isOpen)}>
              <span className={styles.langIconInline}>🌐</span> <span className={styles.langLabel}>{display}</span>
              <svg viewBox="0 0 24 24" width="14" height="14" className={`${styles.chevronIcon} ${isOpen ? styles.chevronRotate : ''}`}><path fill="currentColor" d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z" /></svg>
          </button>
          {isOpen && (
            <div className={styles.langDropdown}>
                <a href={siteConfig.baseUrl} className={`${styles.langItem} ${i18n.currentLocale === 'en' ? styles.langActive : ''}`} onClick={() => setIsOpen(false)}>
                    <span className={styles.dropdownIcon}>🇬🇧</span> English
                </a>
                <a href={`${siteConfig.baseUrl}zh/`} className={`${styles.langItem} ${i18n.currentLocale === 'zh' ? styles.langActive : ''}`} onClick={() => setIsOpen(false)}>
                    <span className={styles.dropdownIcon}>🇨🇳</span> 简体中文
                </a>
            </div>
          )}
      </div>
...
export default function Home() {
    ...
    useEffect(() => {
        const hideNavbarElements = () => {
            ['.navbar__items--right', '.navbar__toggle'].forEach(cls => {
                const el = document.querySelector(cls); if(el) el.style.display = 'none';
            });
        };
        hideNavbarElements();
        const obs = new MutationObserver(hideNavbarElements);
        obs.observe(document.body, { childList: true, subtree: true });

        const revealObs = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) entry.target.classList.add(styles.sectionVisible);
                else entry.target.classList.remove(styles.sectionVisible);
            });
        }, { threshold: 0.1 });
