...
- 📤 **CustomTypeEventRaiser** - GameObject with the raiser script
  - Holds references to 3 events: `DamageInfoGameEvent` for Physical, Fire, and Critical attacks
...
| ------------------ | ----------------------- | ------------------------- |
| `OnPhysicalDamage` | `DamageInfoGameEvent` | Standard physical attacks |
| `OnFireDamage`     | `DamageInfoGameEvent` | Fire-based magical damage |
| `OnCriticalStrike` | `DamageInfoGameEvent` | High-impact critical hits |

**Notice the Behavior Column:**
All three events show **(DamageInfo)** as the type indicator. These `DamageInfoGameEvent` classes were **automatically generated** by the plugin when you created the events—no manual coding required!

:::note 🔧 Code Generation

When you create an event with a custom type in the Game Event Creator, the plugin automatically:

1. Generates the `YourTypeGameEvent` class
...
- The dropdown only shows `DamageInfoGameEvent` assets
- You cannot assign a `StringGameEvent` or `Vector3GameEvent` to these slots
...
{
    [Header("GameEvent")]
    // Notice: DamageInfoGameEvent was AUTO-GENERATED by the plugin
...
**Key Points:**
- 🎯 **Custom Type Support** - `DamageInfoGameEvent` handles complex objects
...
public class CustomTypeEventReceiver : MonoBehaviour
{
    ...
    /// <summary>
    /// Listener method for DamageInfoGameEvent.
...
| --------------------- | ---------------------------------------------------------- |
| 🎯 **Custom Types**    | `YourClassGameEvent` supports any serializable C# class    |
